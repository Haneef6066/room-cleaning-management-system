<?php
  ...
  function getRequestCount($db){
    ...
    $query_request_count = "Select count(*) from cleanrequest cr inner join students s on cr.rollnumber=s.rollnumber where s.hostel='$hostel_name'";
  ...
  function getComplantCount($db){
    $hostel_name = substr($_SESSION['username'], -1);
    $query_complaint_count = "Select count(*) from complaints cr inner join students s on cr.rollnumber=s.rollnumber where s.hostel='$hostel_name'";
  ...
  function getSuggestionCount($db){
    $hostel_name = substr($_SESSION['username'], -1);
    $query_suggestion_count = "Select count(*) from suggestions cr inner join students s on cr.rollnumber=s.rollnumber where s.hostel='$hostel_name'";
  ...
  function getRequests($username, $db){
    ...
    students s Inner Join cleanrequest cr on s.rollnumber = cr.rollnumber 
    ...
    function getComplaints($username, $db){
      ...
      Inner Join students s on c.rollnumber = s.rollnumber
      where s.hostel = '$hostel'
      Order by cr.date desc";
      $result_request = mysqli_query($db, $query_request);
      return $result_request;
    ...
    function getSuggestions($username, $db){
      ...
      Inner Join cleanrequest cr on fb.request_id = cr.request_id
      Inner Join housekeeper hk on cr.worker_id = hk.worker_id
      Inner Join students s on sg.rollnumber = s.rollnumber
      where s.hostel = '$hostel'
      Order by cr.date desc";
      $result_request = mysqli_query($db, $query_request);
      return $result_request;
