<?php
  ...
  require("db.php");
    ...
    if(isset($_POST['regSubmit']) && isset($_SESSION['username'])){
      ...
      $reg_query = "Insert into students values ('$rollnumber', '$password', '$roomnumber', '$floornumber', '$hostel_name')";
    ...
    // Worker assignment handling
    if(isset($_POST['allotSubmit']) && isset($_SESSION['username'])){
      $reqId = mysqli_real_escape_string($db, $_POST['allotId']);
      $workerId = mysqli_real_escape_string($db, $_POST['workerId']);

      $update_query = "UPDATE cleanrequest SET worker_id='$workerId', status=1 WHERE request_id='$reqId'";
      if(mysqli_query($db, $update_query)){
          $_SESSION['success_msg'] = "Worker successfully assigned.";
      } else {
          $_SESSION['error_msg'] = "Error assigning worker: " . mysqli_error($db);
      }
      header("Location: allot.php");
  }
