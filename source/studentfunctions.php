<?php
  ...
  function getStudent($roll, $db){
    $query_find_student = "select * from students where rollnumber='$roll'";
  ...
  function getRequestCount($roll, $db){
    $query_request_count = "Select count(*) as count from cleanrequest where rollnumber='$roll'";
    $result_request_count = mysqli_query($db, $query_request_count);
    $countRow = mysqli_fetch_assoc($result_request_count);
    return $countRow['count']; // Return the count value directly
  }

  // Get Number Of Complaints for a Particular Student
  function getComplaintCount($roll, $db){
    $query_complaint_count = "Select count(*) as count from complaints where rollnumber='$roll'";
  ...
  function getSuggestionCount($roll, $db){
    $query_suggestion_count = "Select count(*) as count from suggestions where rollnumber='$roll'";
  ...
  function getRequests($roll, $db){
    $query_request = "Select cr.request_id as reqid, hk.worker_id, cr.req_status, hk.name, cr.date, cr.cleaningtime, fd.timein, fd.timeout 
                      from cleanrequest cr 
                      Left Join housekeeper hk on cr.worker_id = hk.worker_id 
                      Left Join feedback fd on cr.request_id = fd.request_id 
                      where cr.rollnumber='$roll'";
